...
public class MyAddress implements Iterable<PhoneNumber>, XMLReadWritable {
	...
	*/
	...
	public MyAddress(MyAddress address)
	{
		name = address.name;
	...
	public boolean setBirthday(Calendar birthday)
	{
		...
		if(MyAddress.isValidDay(birthday.get(Calendar.YEAR), 
	...
	{
		// if the day exists, change the birthday
		if(MyAddress.isValidDay(year, month, day))
		{
			// if it is not initialized, initialize it.
			if(this.birthday == null) this.birthday = 
					new Calendar.Builder().build();
	...
	/**
	 * set property from XML element.
	 * @param xmlElement the name of element.
	 * @param value the value of element.
	 */
	private void setPropertyFromXMLElement(String xmlElement, String value) 
			throws MyAddress.WrongSyntaxException
	{
		if(xmlElement.equals(XML_ELEMENT_NAME))
		{
			setName(value);
		}
		
		else if(xmlElement.equals(XML_ELEMENT_PHONE_NUMBERS))
		{
			// split string into the space or comma
			String[] phoneNumberList = value.trim().split("[\\s,]+");
			
			for(String strPhoneNumber : phoneNumberList)
			{
				// if empty string, don't add phone number
				if(strPhoneNumber.isEmpty()) continue;
				
				// add phone number to the list
				try
				{
					if(!addPhoneNumber(new PhoneNumber(strPhoneNumber)))
					{
						// if the phone number is duplicated, throw an exception
						throw new MyAddress.WrongSyntaxException();
					}
				} catch(Exception e)
				{
					// if the phone number is wrong syntax, throw wrong syntax exception
					throw new MyAddress.WrongSyntaxException();
				}
			}
		}
		
		else if(xmlElement.equals(XML_ELEMENT_HOME_ADDRESS))
		{
			setHomeAddress(value);
		}
		
		else if(xmlElement.equals(XML_ELEMENT_OFFICE_ADDRESS))
		{
			setOfficeAddress(value);
		}
		
		else if(xmlElement.equals(XML_ELEMENT_GROUP))
		{
			setGroup(value);
		}
		
		else if(xmlElement.equals(XML_ELEMENT_EMAIL))
		{
			if(!setEmail(value))
			{
				// if the email string is wrong syntax
				throw new MyAddress.WrongSyntaxException();
			}
		}
		
		else if(xmlElement.equals(XML_ELEMENT_URL))
		{
			setURL(value);
		}
		
		else if(xmlElement.equals(XML_ELEMENT_BIRTHDAY))
		{
			String[] addressString = value.split("[/-.]");
			
			// if there doesn't have 3 /, -, or -, throw
			// wrong syntax exception
			if(addressString.length != 3)
				throw new MyAddress.WrongSyntaxException();
			
			try
			{
				int year = Integer.parseInt(addressString[0].trim());
				int month = Integer.parseInt(addressString[1].trim());
				int day = Integer.parseInt(addressString[2].trim());
				
				if(!setBirthday(year, month, day))
				{
					// if the day is not exist, throw wrong syntax exception
					throw new MyAddress.WrongSyntaxException();
				}
			} 
			catch(NumberFormatException e)
			{
				// if string does not have appropriate number format.
				throw new MyAddress.WrongSyntaxException();
			}
		}
		...
		{
			// if the property does not exist, throw wrong syntax
			throw new MyAddress.WrongSyntaxException();
		}
	}
	...
	public void writeToXML(XMLStreamWriter writer) throws XMLStreamException
	{
		...
		if(birthday == null) birthdayString = "";
		else
		{
			birthdayString = birthday.get(Calendar.YEAR) + "/" +
						birthday.get(Calendar.MONTH) + "/" +
						birthday.get(Calendar.DATE);
		}
		...
		{
			if(i != 0) phoneNumberString.append(", ");
			phoneNumberString.append(phoneNumberList.get(i));
		}
		
		// data table for saving to the XML file
		String dataForSaving[][] = {
				{XML_ELEMENT_NAME, name},
				{XML_ELEMENT_PHONE_NUMBERS, phoneNumberString.toString()},
				{XML_ELEMENT_HOME_ADDRESS, homeAddress},
				{XML_ELEMENT_OFFICE_ADDRESS, officeAddress},
				{XML_ELEMENT_GROUP, group},
				{XML_ELEMENT_EMAIL, email},
				{XML_ELEMENT_URL, url},
				{XML_ELEMENT_BIRTHDAY, birthdayString}
		};
		
		writer.writeStartElement("address");
		writer.writeCharacters(System.getProperty("line.separator"));
		
		for(int i=0; i<dataForSaving.length; ++i)
		{
			// if the data is not set, don't write the element
			if(!dataForSaving[i][1].equals(""))
			{
				// write element to the XML
				writer.writeCharacters("\t");
				writer.writeStartElement(dataForSaving[i][0]);
				writer.writeCharacters(" " + dataForSaving[i][1] + " ");
	...
	{	
		// if the node name is not address, it is wrong syntax
		if(xmlAddressElement.getNodeName() != "address")
		{
			throw new MyAddress.WrongSyntaxException();
		}
		
		NodeList nodeList = xmlAddressElement.getChildNodes();
		Node node;
		...
		for(int i=0; i<nodeList.getLength(); ++i)
		{
			...
			if(node.getNodeType() == Node.ELEMENT_NODE)
				setPropertyFromXMLElement(node.getNodeName(), node.getTextContent());
		}
	...
	/**
	* This exception is thrown by MyAddress object if failed to load data from
	* the XML files.
	* 
